...
let previousEnvironmentShellStatuses = new Map<EnvironmentId, EnvironmentShellStatus>();
export const environmentShellStatusesAtom = Atom.make((get) => {
  ...
  if (
    ...
    [...next].every(
      ([environmentId, status]) => previousEnvironmentShellStatuses.get(environmentId) === status,
    )
  ) {
